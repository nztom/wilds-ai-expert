...
- Prefer `tools/save-inspection/Summarize-MHWildsSaveDump.ps1` for normal save questions after a dump exists. It resolves item, monster, endemic-life, and fish names from local Wilds assets and emits CSVs to the summary dir root and JSON to a `json/` subfolder. Always read the CSVs first and answer from them; if detail is missing, tell the user what was found and offer to check the JSON files — do not open `json/` unless the user asks.- Always update `tools/ree-save-editor/` from its configured branch before building it.
